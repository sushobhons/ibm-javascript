...
function bookSeat( row, col){
    if(theaterSeats[row][col]==='0'){
        ...
        alert(`Seat ${String.fromCharCode(65 + 1)}${col + 1} is booked.`);
    }else{
        alert(`Seat ${String.fromCharCode(65 + 1)}${col + 1} is already taken.`);
    }
}
...
function updateSeatStatus(row, col, status){
    ...
    seats[index].classList.remove('booked', 'available');
